...
def login():
    ...
    user_info = User.query.filter_by(login_name=login_name).filter_by(type=0).first()
    if not user_info:
        resp['code'] = -1
        resp['msg'] = "您的账号信息不存在~~"
        return jsonify(resp)
...
def resetPwd():
    ...
    response.set_cookie(app.config['AUTH_COOKIE_NAME'], '%s#%s' % (UserService.geneAuthCode(user_info), user_info.uid), 60 * 60 * 24 * 1)  # 保存1天
    return response
